...
public class EnableAlertUdpateSlashCommand implements SlashCommandHandler {
    ...
    public EnableAlertUdpateSlashCommand(
    ...
    public void handle(SlashCommandInteractionEvent event) throws Exception {
        this.serverUpdateAlertRepository.save(
                new ServerUpdateAlertEntity(Objects.requireNonNull(event.getGuild()).getIdLong()));
        event.getHook()
                .sendMessageEmbeds(
                        EmbedFactory.create(
                                        "Fonctionnalité activé",
                                        "Les alertes de mise à jour de calendrier ont été activées avec succès !",
                                        Color.green,
                                        Map.of())
                                .build())
                .queue();
